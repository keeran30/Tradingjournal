...
export default function PsychologyPage() {
  ...
  // ─── FETCH TRADES — FILTERED BY CURRENT USER ────────
  ...
  const exportAsText = () => {
    ...
    let txt="TRADEVAULT PSYCHOLOGY REPORT\n\n"
  ...
  const exportAsHTML = () => {
    ...
    emotionStats.forEach((e,i)=>{
      er+=`<tr style="border-bottom:1px solid #333"><td style="padding:8px">${i+1}</td><td style="padding:8px">${e.emoji} ${e.emotion}</td><td style="padding:8px;text-align:center">${e.count}</td><td style="padding:8px;text-align:center;color:${e.winRate>=50?"#22c55e":"#ef4444"}">${e.winRate}%</td><td style="padding:8px;text-align:center;color:${e.totalPnL>=0?"#22c55e":"#ef4444"}">$${e.totalPnL.toFixed(2)}</td></tr>`
    })
    const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>TradeVault Psychology</title><style>body{font-family:Arial;background:#111;color:#eee;padding:40px;max-width:800px;margin:0 auto}h1{color:#eab308;text-align:center}.stat{text-align:center;background:#1a1a1a;padding:15px;border-radius:10px;display:inline-block;margin:10px}.stat .v{font-size:24px;font-weight:bold;color:#eab308}.stat .l{font-size:11px;color:#888}table{width:100%;border-collapse:collapse;margin-top:20px}th{background:#1a1a1a;padding:10px;text-align:left;font-size:11px;color:#888;border-bottom:2px solid #eab308}td{font-size:13px}.footer{text-align:center;margin-top:40px;color:#555;font-size:10px}</style></head><body><h1>TRADEVAULT</h1><p style="text-align:center;color:#888">Psychology Report · ${rd}</p><div style="text-align:center"><div class="stat"><div class="v">${emotionalScore}/100</div><div class="l">Emotional Score</div></div><div class="stat"><div class="v">${trades.length}</div><div class="l">Total Trades</div></div></div><table><thead><tr><th>#</th><th>Emotion</th><th>Trades</th><th>Win Rate</th><th>Total P&L</th></tr></thead><tbody>${er}</tbody></table><p class="footer">Generated by TradeVault.pro</p></body></html>`
    const blob=new Blob([html],{type:"text/html"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`TradeVault_Psychology_${rd}.html`;a.click();URL.revokeObjectURL(url)
    showNotification("Downloaded as HTML (open in browser)","success")
  }
  ...
  const exportReport = () => {
    ...
    if (premium) {
      if (window.confirm("Click OK for HTML Report, Cancel for TXT")) exportAsHTML()
      else exportAsText()
    } else {
      exportAsText()
    }
        ...
        {activeTab==="report"&&(
          <div className="space-y-6">
            ...
            <div className="text-center">
              <button onClick={exportReport} className="bg-yellow-500 text-black px-8 py-3.5 rounded-xl font-bold">📥 Download Report</button>
            </div>
          </div>
        )}
