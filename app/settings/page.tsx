...
export default function SettingsPage() {
  ...
  const handleDeleteAccount = async () => {
    const confirmed = window.confirm(
      "Are you sure you want to delete your account? This cannot be undone. All your trades will be permanently deleted."
    )
    if (!confirmed) return
  ...
  const handleCancelSubscription = async () => {
    const confirmed = window.confirm("Are you sure you want to cancel your Premium subscription?")
    if (!confirmed) return
      ...
      <section className="flex-1 p-4 md:p-8 overflow-y-auto">
        {message && (
          <div className={`fixed top-4 right-4 z-50 px-6 py-3 rounded-xl font-bold shadow-lg animate-bounce ${
            ...
            message.includes("cancelled") ? "bg-yellow-600 text-white" : "bg-blue-600 text-white"
          }`}>
            {message}
          </div>
        )}
